...
export const handler = async (event) => {
  ...
  const oldAbilityList =
    event.Records[0].dynamodb?.OldImage?.abilityList?.L?.map(
      (ability) => ability.M,
    );
  ...
  try {
    ...
    const pushMessageTitle = "アビリティ変更通知";
    const pushMessageBody = "アビリティの設定を変更しました";
    const notificationType = "abilitySetting";
